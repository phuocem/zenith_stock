...
class CreateTransactionView extends GetView<TransactionController> {
        ...
        children: [
          ...
          Expanded(
            child: Obx(() => DropdownButton<Warehouse>(
              ...
              hint: const Text("Chọn kho", style: TextStyle(color: Colors.white54)),
      ...
      if (controller.draftItems.isEmpty) {
        return Container(
          ...
          child: const Center(
            child: Column(
              children: [
                Icon(Icons.add_shopping_cart_outlined, size: 40, color: Colors.white12),
                SizedBox(height: 12),
                Text("Chưa có sản phẩm nào\nNhấn nút bên dưới để thêm", style: TextStyle(color: Colors.white38, fontSize: 13), textAlign: TextAlign.center),
              ],
            ),
          ),
        );
        ...
        children: [
          SectionHeader(
            ...
            trailing: TextButton(
              ...
              child: const Text("Xóa tất cả", style: TextStyle(color: AppTheme.dangerColor, fontSize: 12)),
            ),
          ),
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(item.product.name, style: AppTheme.titleStyle.copyWith(fontSize: 14), maxLines: 1, overflow: TextOverflow.ellipsis),
                          const SizedBox(height: 4),
                          Text("Lô: ${item.batch.batchCode}", style: AppTheme.captionStyle.copyWith(fontSize: 11)),
                          if (isOut)
                            Text("Còn: ${item.batch.currentQuantity}", style: AppTheme.captionStyle.copyWith(fontSize: 11, color: AppTheme.warningColor)),
                        ],
                      ),
                    ),
  ...
  Widget _buildAddProductBtn(BuildContext context, bool isIn, Color color) {
    return GestureDetector(
      ...
      child: Container(
        ...
        padding: const EdgeInsets.all(16),
        decoration: AppTheme.cardDecoration(borderColor: color.withOpacity(0.3)),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.add_circle_outline, color: color, size: 22),
            const SizedBox(width: 10),
            Text("Thêm sản phẩm vào phiếu", style: TextStyle(color: color, fontWeight: FontWeight.bold, fontSize: 14)),
          ],
        ),
      ),
    );
  ...
  Widget _buildSubmitBar(Color color, bool isIn, TextEditingController notes, TextEditingController ref) {
    return Container(
      ...
      child: Obx(() => ZenithButton(
        ...
        onPressed: () => controller.submitTransaction(notes: notes.text, refNumber: ref.text),
      )),
    );
  }
...
class _AddItemSheetState extends State<_AddItemSheet> {
  ...
  Future<void> _onProductSelected(Product p) async {
    ...
    try {
      ...
      setState(() { _batches = list; if (list.isNotEmpty) _selBatch = list.first; _loading = false; });
    } catch (_) {
      setState(() => _loading = false);
    }
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(widget.isIn ? Icons.south_west_rounded : Icons.north_east_rounded, color: color),
              const SizedBox(width: 10),
              Text("Chọn sản phẩm", style: AppTheme.headlineStyle.copyWith(fontSize: 18)),
            ],
          ),
          const SizedBox(height: 20),

          if (_loading) const Expanded(child: Center(child: CircularProgressIndicator(color: AppTheme.primaryColor)))
          else Expanded(
            child: SingleChildScrollView(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  ...
                  DropdownButtonFormField<Product>(
                    ...
                    items: _products.map((p) => DropdownMenuItem(
                      value: p,
                      child: Text('${p.name} (Còn: ${p.currentStock})', style: const TextStyle(fontSize: 13)),
                    )).toList(),
                  ...
                  if (_selProduct != null) ...[
                    DropdownButtonFormField<Batch>(
                      ...
                      items: _batches.map((b) => DropdownMenuItem(
                        value: b,
                        child: Text('${b.batchCode} (Còn: ${b.currentQuantity})', style: const TextStyle(fontSize: 13)),
                      )).toList(),
                  ...
                  if (_selBatch != null) ...[
                    ...
                    Row(
                      children: [
                        IconButton(
                          icon: const Icon(Icons.remove_circle, color: AppTheme.dangerColor),
                        ...
                        Expanded(
                          child: TextField(
                            ...
                            style: AppTheme.numberStyle.copyWith(fontSize: 24),
                    ...
                    if (!widget.isIn && _qty > (_selBatch!.currentQuantity))
                      Padding(
                        ...
                        child: ZenithBadge(label: "⚠ Vượt tồn kho (${_selBatch!.currentQuantity})", color: AppTheme.warningColor),
                      ),
                  ],
                ],
              ),
          ...
          ZenithButton(
            ...
            gradient: widget.isIn ? AppTheme.successGradient : AppTheme.dangerGradient,
